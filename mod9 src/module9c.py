...
parser.add_argument('--rps', type=float, default=4, help='rps')
parser.add_argument('--timRun', type=float, default=4, help='time to run (sec)')
...
while cur_time - start_time < args.timRun:
   ...
   if (cur_time - msg_time > args.timSamples):
      ...
      if AD_reading[index] > 350 and AD_reading[index] < 650:
...
# Writing to a file
with open('data_9c.txt','w') as f:
    for i in range(0,len(time_array)-1):
        f.write(f'{time_array[i]:.5f}\t{RPSs[i]:.3f}\t{AD_reading[i]:.3f}\n')
